		...
		let device = Devices.findAll({
			...
			sql.query(query, async function (err, resp) {
				...
				if (resp.affectedRows) {
					var code = randomize("0", 7);
					var activation_code = await helpers.checkActivationCode(
						code
					);
					// let device_id_new = helpers.getDeviceId();
					// device_id_new = await helpers.checkDeviceId(device_id_new);
					...
					res.send({
						...
						data: {
							status: false,
							msg: await helpers.convertToLang(
								req.translation[MsgConstants.ERR_TRANSF],
								"Error While Transfere"
							) // Error While Transfere.
						}
					});
				}
	...
	if (verify) {
			...
			var rest = sql.query(sql1, async function (error, results) {
				if (error) {
					...
					data = {
						status: false,
						msg: await helpers.convertToLang(
							req.translation[MsgConstants.DEVICE_NOT_UNFLAG],
							"Device not Unflagged.Please try again"
						) // Device not Unflagged.Please try again.
					};
					res.send(data);
				} else {
		...
		data = {
			...
			msg: await helpers.convertToLang(
				req.translation[MsgConstants.DEVICE_NOT_UNFLAG],
				"Device not Unflagged.Please try again"
			) // Device Is not unflagged.Please try again"
		};
		res.send(data);
	}
};
			...
			if (
				...
				if (results.affectedRows == 0) {
					data = {
						status: false,
						msg: await helpers.convertToLang(
							req.translation[MsgConstants.DEVICE_NOT_FLAG],
							"Device not Flagged.Please try again"
						) // Device not Flagged.Please try again."
					};
				} else {
					...
					if (resquery.length) {
						...
						data = {
							...
							status: true,
							msg: await helpers.convertToLang(
								req.translation[MsgConstants.DEVICE_FLAG_SUCC],
								"Device Flagged successfully"
							) // Device Flagged successfully."
						};
				...
				data = {
					status: false,
					msg: await helpers.convertToLang(
						req.translation[MsgConstants.DEVICE_ALRDY_FLAG],
						"Device Already Flagged"
					) // Device Already Flagged
				};
				res.send(data);
			}
...
>>>>>>> 183e19ab54b9d524ba77b4997c47e5176b60384a
};
...
exports.applySettings = async function (req, res) {
    try {
        ...
        if (verify) {
            ...
            if (type == "profile") {
                applyQuery =
                    "INSERT INTO device_history (device_id, dealer_id, user_acc_id, profile_name, app_list, passwords, controls, permissions, type) VALUES ('" +
                    device_id +
                    "', " +
                    dealer_id +
                    ", " +
                ...
                applyQuery =
                    "INSERT INTO device_history (device_id, dealer_id, user_acc_id, app_list, passwords, controls, permissions) VALUES ('" +
                    device_id +
                    "', " +
                    dealer_id +
                    ", " +
            ...
            sql.query(applyQuery, async function (err, rslts) {
                ...
                if (rslts && rslts.insertId) {
                    ...
                    if (isOnline) {
                        ...
                        await device_helpers.updateExtensions(
                            extensions,
                            device_id
                        );
...
exports.applyPullApps = async function (req, res) {
    try {
        ...
        if (verify) {
            ...
            var applyQuery =
                "INSERT INTO device_history (device_id,dealer_id,user_acc_id, pull_apps, type) VALUES ('" +
                device_id +
                "'," +
                dealer_id +
                "," +
