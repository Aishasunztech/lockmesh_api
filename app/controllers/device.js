                                            ...
                                            if (success.length || checkDevicepgp.length) {
                                                ...
                                                sql.query(common_Query, async function (commonQueryError, result) {
                                                    ...
                                                    if (term === '0') {
                                                        ...
                                                        let remaining_credits = dealer_credits - total_price
                ...
                if (result.length || req.body.term === '0') {
                    ...
                    if (dealer_credits > total_price || req.body.term === '0') {
                        if (products.length || packages.length) {
                            if (duplicate > 0) {
                                if (dealer_credits > total_price * duplicate || req.body.term === '0') {
                                    ...
                                    let remaining_credits = dealer_credits
                                    if (req.body.term !== '0') {
                                        remaining_credits = dealer_credits - total_price * duplicate
                                        let deduct_credits = 'update dealer_credits set credits =' + remaining_credits + ' where dealer_id ="' + dealer_id + '"';
                                        await sql.query(deduct_credits);
        ...
        if (verify) {
            ...
            for (let device of req.body.devices) {
                if (action == 'unlink') {
                    ...
                    // let result = await sql.query(getdevicePkgs);
                    ...
                    if (bills.length) {
                        ...
                        let updateBilling = "UPDATE services_billing set del_status = '1' AND deleted_at = '" + currentDate + "' WHERE user_acc_id = " + user_acc_id;
            ...
            if (action == 'pre-active' && refundedCredits !== 0) {
