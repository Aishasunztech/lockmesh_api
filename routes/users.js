...
router.post('/create/device_profile', async function (req, res) {
    ...
    if (verify.status !== undefined && verify.status == true) {
        ...
        if (duplicate > 0) {
            ...
            data = {
                "status": true,
                "msg": await helpers.convertToLang(loggedInuid, MsgConstants.PRE_ACTIV_ADD_SUCC_EMAIL_SEND), // Pre-activations added succcessfully.Email sends to your account.
                "data": rsltq
            };
            res.send({
                status: true,
                                ...
                                sql.query(insertUser_acc, async (err, resp) => {
                                    ...
                                    if (resp && resp.affectedRows) {
                                        ...
                                        sql.query("select devices.*  ," + usr_acc_query_text + ", dealers.dealer_name, dealers.connected_dealer from devices left join usr_acc on devices.id = usr_acc.device_id LEFT JOIN dealers on usr_acc.dealer_id = dealers.dealer_id WHERE usr_acc.transfer_status = 0 and devices.id='" + dvc_id + "'", async function (error, results, fields) {
                                            ...
                                            data = {
                                                "status": true,
                                                "msg": await helpers.convertToLang(loggedInuid, MsgConstants.PRE_ACTIV_ADD_SUCC), // Pre-activation added succcessfully.
                                                "data": results
                                            };
                                        ...
                                        res.send({
                                            status: false,
                                            msg: await helpers.convertToLang(loggedInuid, MsgConstants.DEVICE_NOT_ADD), // Device couldn't added
                                        });
                                        return;
                                    }
