...
router.post("/login", async function(req, res) {
  ...
  var sql1 =
    "SELECT * FROM dealers WHERE dealer_email = '" + email + "' limit 1";
  ...
  if (users.length == 0) {
    ...
    var userTypeQuery =
      "SELECT * FROM user_roles WHERE id =" + users[0].type + " AND status='1'";
    ...
    if (userType.length == 0) {
      ...
      var dUser =
        "SELECT * FROM dealers WHERE  dealer_email='" +
        email +
        "' and password ='" +
        enc_pwd +
        "';";
...
router.get("/devices", function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    sql.query(
      "select devices.*  ," +
        usr_acc_query_text +
        ", dealers.dealer_name,dealers.connected_dealer from devices left join usr_acc on  devices.id = usr_acc.device_id LEFT JOIN dealers on usr_acc.dealer_id = dealers.dealer_id WHERE usr_acc.transfer_status = 0 AND devices.reject_status = 0 " +
        where_con +
...
router.get("/new/devices", function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    sql.query(
      "select devices.*  ," +
        usr_acc_query_text +
        ' from devices left join usr_acc on  devices.id = usr_acc.device_id WHERE ((usr_acc.device_status=0 OR usr_acc.device_status="0") AND (usr_acc.unlink_status=0 OR usr_acc.unlink_status="0") AND (usr_acc.activation_status IS NULL)) AND devices.reject_status = 0 ' +
        where_con +
        " order by devices.id DESC",
      function(error, results, fields) {
        if (error) throw error;
...
/*** Add Device History ***/
router.post("/add/device_history", async function(req, res) {
//   res.setHeader("Content-Type", "application/json");
//   console.log(req.body);
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    console.log("abaid: At server side device id is:", req.body[0]);
//  console.log('length is: ', req.body.length);

    var action = "DELETE";
    var device_id = req.body[i].device_id;
    var device_name = req.body[i].device_name;
    var session_id = req.body[i].session_id;
    var model = req.body[i].model;
    var ip_address = req.body[i].ip_address;
    var simno = req.body[i].simno;
    var imei = req.body[i].imei;
    var simno2 = req.body[i].simno2;
    var imei2 = req.body[i].imei2;
    var serial_number = req.body[i].serial_number;
    var mac_address = req.body[i].mac_address;
    var fcm_token = req.body[i].fcm_token;
    var online = req.body[i].online;
    var is_sync = req.body[i].is_sync;
    var flagged = req.body[i].flagged;
    var screen_start_date = req.body[i].screen_start_date;
    var reject_status = req.body[i].reject_status;
    var account_name = req.body[i].account_name;
    var account_email = req.body[i].account_email;
    var dealer_id = req.body[i].dealer_id;
    var prnt_dlr_id = req.body[i].prnt_dlr_id;
    var link_code = req.body[i].link_code;
    var client_id = req.body[i].client_id;
    var start_date = req.body[i].start_date;
    var expiry_months = req.body[i].expiry_months;
    var expiry_date = req.body[i].expiry_date;
    var activation_code = req.body[i].activation_code;
    var status = req.body[i].status;
    var device_status = req.body[i].device_status;
    var activation_status = req.body[i].activation_status;
    var wipe_status = req.body[i].wipe_status;
    var account_status = req.body[i].account_status;
    var unlink_status = req.body[i].unlink_status;
    var transfer_status = req.body[i].transfer_status;
    var dealer_name = req.body[i].dealer_name;
    var prnt_dlr_name = req.body[i].prnt_dlr_name;
    var user_acc_id = req.body[i].user_acc_id;
    var pgp_email = req.body[i].pgp_email;
    var chat_id = req.body[i].chat_id;
    var sim_id = req.body[i].sim_id;
    var finalStatus = req.body[i].finalStatus;

    // cosonsole.log('wipe status: ');
    // return
    var sql1 =
      "INSERT INTO acc_action_history (action, device_id, device_name, session_id, model, ip_address, simno, imei, simno2, imei2, serial_number,  mac_address, fcm_token, online, is_sync, flagged, screen_start_date, reject_status, account_name, account_email, dealer_id, prnt_dlr_id, link_code, client_id, start_date, expiry_months, expiry_date, activation_code, status, device_status, activation_status, wipe_status, account_status, unlink_status, transfer_status,  dealer_name, prnt_dlr_name, user_acc_id, pgp_email, chat_id, sim_id, finalStatus,created_at, updated_at)";
    ...
    // console.log("insert query of device history abaid: " + sql1);

    sql.query(sql1, function(error, rows) {
      if (error) throw error;
      console.log("insert query successfully run");
    });

    // console.log('check id: ', req.body[0].usr_device_id);
    var sql2 =
      "UPDATE usr_acc SET del_status= '" +
      1 +
      "' WHERE device_id = " +
      req.body[i].usr_device_id;

    // console.log("update query of device history abaid: " + sql2);

    sql.query(sql2, function(error, rows) {
        if (error) throw error;
        console.log("Update query successfully run");
      });
...
router.post("/add/dealer", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    if (!empty(dealerEmail) && !empty(dealerName)) {
      var dealer = await sql.query(
        "SELECT * FROM dealers WHERE dealer_email = '" + dealerEmail + "'"
      );
      ...
      var sql1 =
        "INSERT INTO dealers (connected_dealer, dealer_name, dealer_email, password, link_code , type , modified, created)";
      if (
        sdealerDealerId != undefined &&
        !empty(sdealerDealerId) &&
        sdealerDealerId != null &&
        sdealerDealerId != 0
      ) {
        sql1 +=
          " VALUES (" +
      ...
      sql.query(sql1, function(error, rows) {
        ...
        sendEmail("Account Registration", html, dealerEmail, async function(
          errors,
          response
        ) {
          if (error) {
    ...
    if (role) {
      sql.query(
        ...
        async function(error, results) {
          ...
          for (var i = 0; i < results.length; i++) {
            if (
              results[i].connected_dealer != 0 &&
              results[i].connected_dealer != "" &&
              results[i].connected_dealer != "0"
            ) {
              var get_parent_dealer = await sql.query(
                "select dealer_id, dealer_name from dealers where dealer_id=" +
                  results[i].connected_dealer +
                  " limit 1"
              );
              console.log(get_parent_dealer);
            }
            var get_connected_devices = await sql.query(
              "select count(*) as total from usr_acc where dealer_id='" +
                results[i].dealer_id +
                "'"
            );
            ...
            if (
              get_parent_dealer != undefined &&
              get_parent_dealer.length > 0
            ) {
...
router.get("/dealers", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    if (verify.user.user_type == "admin") {
      ...
      sql.query(
        "select * from dealers where type!=" + role + " order by created DESC",
        async function(error, results) {
          if (error) throw error;
...
router.post("/create/device_profile", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    let checkUnique =
      "SELECT account_email from usr_acc WHERE account_email= '" + email + "'";
    let checkUniquePgp =
      ...
      sql.query(checkDealer, async (error, response) => {
        ...
        if (response.length) {
          ...
          sql.query(insertDevice, async (err, resp) => {
            ...
            var insertUser_acc =
              "INSERT INTO usr_acc (device_id, activation_code, client_id , account_email,expiry_months, dealer_id, device_status, activation_status  ";
              ...
              sql.query(insertUser_acc, async (err, resp) => {
                ...
                if (resp.affectedRows) {
                  ...
                  if (policy_id !== "") {
                    ...
                    var insertQuery =
                      "INSERT INTO device_history ( user_acc_id, app_list, setting, controls, status ) " +
                      " VALUES('" +
                      user_acc_id +
                      "', '" +
                      policy_obj[0].app_list +
                      "', '" +
                      policy_obj[0].setting +
                      "', '" +
                      policy_obj[0].controls +
                      "', 0 ) ";
                  ...
                  sql.query(
                    "select devices.*  ," +
                      usr_acc_query_text +
                      ", dealers.dealer_name, dealers.connected_dealer from devices left join usr_acc on devices.id = usr_acc.device_id LEFT JOIN dealers on usr_acc.dealer_id = dealers.dealer_id WHERE usr_acc.transfer_status = 0 and devices.device_id='" +
                      device_id +
                      "'",
                    async function(error, results, fields) {
                      if (error) throw error;
  ...
  if (verify["status"] !== undefined && verify.status === true) {
    ...
    let device = Devices.findAll({
      ...
      var value =
        ...
        new_object.email +
        "', '" +
        new_object.pgp_email +
        "', '" +
        new_object.expiry_months +
        "', '" +
        new_object.dealer_id +
        "', 0, 0 , '" +
        ...
        new_object.imei2 +
        "', '" +
        new_object.serial_number +
        "', '" +
        new_object.mac_address +
        "', '" +
        new_object.s_dealer +
        "', '" +
        new_object.s_dealer_name +
        "', '" +
        new_object.account +
        "', '" +
        new_object.fcm_token +
        "', '" +
        new_object.account_status +
        "', '" +
        new_object.start_date +
        "', '" +
        new_object.expiry_date +
        "', '" +
        new_object.link_code +
        "' )";
      ...
      sql.query(query, async (err, resp) => {
        ...
        if (resp.affectedRows) {
          ...
          let updateprevDevice =
            'update devices set transfer_status=1, email=null, pgp_email=null, chat_id=null, sim_id=null where device_id ="' +
            device_id +
            '"';
          await sql.query(updateprevDevice);

          console.log("id", device_id);
          let insertpreActiveD =
            "insert into devices (device_id, activation_code, name, client_id, chat_id, model, email, pgp_email, expiry_months, dealer_id, device_status, activation_status)";
          let values =
            "values('" +
            device_id_new +
            "', '" +
    ...
    if (!empty(device_id)) {
      ...
      sql.query(checkDevice, async function(error, rows) {
        if (rows.length) {
          ...
          let checkUnique =
            "SELECT usr_acc.* from usr_acc WHERE account_email= '" +
            device_email +
            "' AND device_id != '" +
            device_id +
            "'";
          sql.query(checkUnique, async (error, success) => {
            if (success.length || checkDevicepgp.length) {
              ...
              if (connected_dealer !== 0) {
                ...
                // let common_Query2 = " status = '" + status + "', device_status = 1, unlink_status=0 ,  start_date = '" + start_date + "' ,expiry_date = '" + expiry_date + "' where device_id = '" + device_id + "'";
                usr_acc_Query =
                  "UPDATE usr_acc set account_email = '" +
                  device_email +
                  "', status = '" +
                  status +
              ...
              sql.query(common_Query, async function(error, result) {
                ...
                var slctquery =
                  "select devices.*  ," +
                  usr_acc_query_text +
                  ", dealers.dealer_name,dealers.connected_dealer , pgp_emails.pgp_email,chat_ids.chat_id ,sim_ids.sim_id from devices left join usr_acc on  devices.id = usr_acc.device_id left join dealers on dealers.dealer_id = usr_acc.dealer_id LEFT JOIN pgp_emails on pgp_emails.user_acc_id = usr_acc.id LEFT JOIN chat_ids on chat_ids.user_acc_id = usr_acc.id LEFT JOIN sim_ids on sim_ids.device_id = usr_acc.device_id where devices.device_id = '" +
                  device_id +
                  "'";
                console.log(slctquery);
                rsltq = await sql.query(slctquery);
                ...
                if (
                  ...
                  var slctpolicy =
                    "select * from device_history where id = " + policy_id + "";
                  policy_obj = await sql.query(slctpolicy);
                  var insertQuery =
                    "INSERT INTO device_history ( device_id, app_list, setting, controls, passwords, type, status ) " +
                    " VALUES('" +
                    device_id +
                    "', '" +
                    policy_obj[0].app_list +
...
router.put("/edit/devices", async function(req, res) {
  ...
  if (verify["status"] !== undefined && verify.status == true) {
    ...
    if (!empty(req.body.device_id)) {
      ...
      var checkDevice =
        "SELECT start_date from usr_acc WHERE device_id = '" +
        usr_device_id +
        "' ";
      if (loggedDealerType === SDEALER) {
        checkDevice = checkDevice + " AND dealer_id = " + loggedDealerId;
...
router.put("/delete/:device_id", function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    if (!empty(req.params.device_id)) {
      ...
      console.log(
        "delete where ",
        'DELETE FROM devices WHERE device_id ="' + [req.params.device_id]
      );
      if (
        ...
        sql.query(
          'UPDATE devices set reject_status = 1 WHERE device_id ="' +
...
router.put("/updateProfile/:id", function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    sql.query(
      "UPDATE dealers SET `dealer_name` = ? where `dealer_id` = ?",
      [req.body.name, req.body.dealerId],
      function(error, rows, status) {
        if (error) throw error;
        if (status) {
          data = {
            status: false,
...
router.post("/unlink/:id", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    if (!empty(device_id)) {
      let query =
        "SELECT activation_code from usr_acc where device_id = '" +
        device_id +
        "'";
      ...
      if (result[0].activation_code !== null) {
        var sql1 =
          "update usr_acc set dealer_id = null, activation_status=0, status = '' , device_status = 0 , start_date= '', expiry_date= '' , unlink_status=1 where device_id = '" +
          device_id +
          "'";
      } else {
        var sql1 =
          "update usr_acc set dealer_id = null, status = '' , device_status = 0 , start_date= '', expiry_date= '' , unlink_status=1 where device_id = '" +
          device_id +
          "'";
      }
            ...
            sql.query(
              ...
              async function(error, resquery, fields) {
                ...
                if (resquery.length) {
                  ...
                  require("../bin/www").sendDeviceStatus(
                    gtres[0].device_id,
                    "suspended"
                  );
...
router.post("/flagDevice/:id", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    if (!empty(device_id)) {
      if (
        gtres[0].flagged === "" ||
        gtres[0].flagged === "null" ||
        gtres[0].flagged === null
      ) {
        var sql1 =
          "update devices set flagged='" +
          option +
          "' where id = '" +
          device_id +
          "'";
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    if (req.body.pageName != undefined && req.body.pageName != "") {
      ...
      if (user.user_type === ADMIN || user.user_type === DEALER) {
        ...
        isReset = true;
        var query =
          "SELECT password FROM dealers WHERE dealer_id=" +
          req.body.dealer_id +
          " limit 1";
    ...
    if (!empty(email) && !empty(newpwd) && !empty(dealer_id)) {
      var query =
        "SELECT link_code from dealers where dealer_id=" +
        dealer_id +
        " AND password='" +
        curntPassword +
        "' limit 1";
      ...
      if (result.length) {
        ...
        if (isReset) {
          ...
          var message =
            "Your login details are : <br> Email : " +
            email +
            "<br> Password : " +
            newpwd +
            "<br> Dealer id : " +
            ...
            "<br> Dealer Pin : " +
            result[0].link_code +
            ".<br> Below is the link to login : <br> http://www.lockmesh.com <br>";
        } else {
          var subject = "Password Change";
          var message =
            "You have changed your password in your Lockmesh.com account. <br><br> This is just to inform you about the activity. If it was not you, please immediately contact your provider to reset the password.";
        }
  ...
  if (verify.status !== undefined && verify.status == true) {
    if (!empty(dealer_id) && (!empty(name) || !empty(email))) {
      if (!empty(email) && email != "" && email != null) {
        mailgiven = true;
        let checkMail = await sql.query(
          "select dealer_id from dealers where dealer_email='" +
            email +
            "' and dealer_id !=" +
            dealer_id
        );
...
router.post("/dealer/delete/", function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    if (!empty(dealer_id)) {
      var qury =
        "UPDATE dealers set unlink_status = 1 where dealer_id = '" +
        dealer_id +
        "'";

      sql.query(qury, async function(error, row) {
        var qury1 =
          "UPDATE dealers set unlink_status = 1 where connected_dealer = '" +
          dealer_id +
          "'";
...
router.post("/dealer/suspend", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    if (!empty(dealer_id)) {
      ...
      //suspend dealer
      var qury =
        "UPDATE dealers set account_status = 'suspended' where dealer_id = '" +
        dealer_id +
        "'";
      ...
      sql.query(qury, async function(error, row) {
        //suspend sub dealer
        var qury1 =
          "UPDATE dealers set account_status = 'suspended' where connected_dealer = '" +
          dealer_id +
          "'";
        var rslt = await sql.query(qury1);
        console.log(qury1);
...
router.post("/dealer/activate", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    if (!empty(dealer_id)) {
      var qury =
        "UPDATE dealers set account_status = '' where dealer_id = '" +
        dealer_id +
        "'";

      sql.query(qury, async function(error, row) {
        var qury1 =
          "UPDATE dealers set account_status = '' where connected_dealer = '" +
          dealer_id +
          "'";
        var rslt = await sql.query(qury1);
        if (row.affectedRows != 0 && rslt.affectedRows != 0) {
...
router.get("/sdealers/:dealer_id", function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    sql.query(
      "select * from dealers where connected_dealer = '" +
        req.params.dealer_id +
        "' and type='sdealer' order by created DESC",
      async function(error, results) {
        if (error) throw error;
        //console.log(results.length);
        var data = [];
        for (var i = 0; i < results.length; i++) {
          var get_connected_devices = await sql.query(
            "select count(*) as total from devices where dealer_id='" +
              results[i].dealer_id +
              "'"
          );
...
router.post("/dealer/undo", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    if (!empty(dealer_id)) {
      var qury =
        "UPDATE dealers set unlink_status = '0' where dealer_id = '" +
        dealer_id +
        "'";

      sql.query(qury, async function(error, row) {
        var qury1 =
          "UPDATE dealers set unlink_status = '0' where connected_dealer = '" +
          dealer_id +
          "'";
        var rslt = await sql.query(qury1);
      ...
      await sql.query(
        ...
        async function(error, results) {
          ...
          if (results.length == 0) {
            ...
            var query =
              "select * from dealers where dealer_id =" + results[0].dealer_id;
...
router.get("/get_apps/:device_id", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    if (!empty(req.params.device_id)) {
      var query =
        'SELECT user_apps.*, apps_info.label, apps_info.unique_name as uniqueName, apps_info.icon as icon from user_apps LEFT JOIN apps_info on user_apps.app_id = apps_info.id LEFT JOIN devices on user_apps.device_id=devices.id where devices.device_id ="' +
        req.params.device_id +
        '"';
      // console.log(query);
      try {
...
router.get("default_apps", function(req, res) {
  ...
  if (verify["status"] !== undefined && verify.status === true) {
    var query =
      "SELECT apps_info.label, apps_info.unique_name as uniqueName, apps_info.icon as icon from default_apps as apps_info ";
    // console.log(query);
    sql.query(query, async (error, apps) => {
      if (error) {
        throw Error("Query Expection");
...
router.post("/apply_settings/:device_id", async function(req, res) {
  try {
    ...
    if (verify.status !== undefined && verify.status == true) {
      ...
      let app_list =
        req.body.device_setting.app_list == undefined
          ? ""
          : JSON.stringify(req.body.device_setting.app_list);
      // console.log("controls: " + app_list);

      let passwords =
        req.body.device_setting.passwords == undefined
          ? ""
          : JSON.stringify(req.body.device_setting.passwords);
      ...
      if (type === "profile") {
        var query =
          "select id from usr_acc_profile where profile_name = '" + name + "'";
        let result = await sql.query(query);

        if (result.length == 0 || name == "") {
          var applyQuery =
            "insert into usr_acc_profile (profile_name, user_acc_id, app_list, setting, controls,passwords, type) values ('" +
            name +
            "', " +
            usr_acc_id +
            ",'" +
        ...
        if (result.length == 0 || name == "") {
          var applyQuery =
            "insert into policy (policy_name, app_list, setting, controls,passwords) values ('" +
            name +
            "','" +
            app_list +
            "', null, '" +
        ...
        var applyQuery =
          "insert into device_history (user_acc_id, app_list, setting, controls) values ('" +
          usr_acc_id +
          "','" +
          app_list +
          "', null, '" +
...
router.post("/dealer/dropdown", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    var squery =
      "select * from dealer_dropdown_list where dealer_id = " +
      dealer_id +
      " AND type ='" +
      dropdownType +
      "'";
    ...
    if (srslt.length == 0) {
      var squery = sql.query(
        "insert into dealer_dropdown_list (dealer_id, selected_items, type) values (" +
          dealer_id +
          ", '" +
          selected_items +
          "', '" +
      ...
      sql.query(
        "update dealer_dropdown_list set selected_items = '" +
          selected_items +
          "' where type='" +
          dropdownType +
          "' AND dealer_id='" +
...
router.post("/dealer/postPagination", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    var dealer_id = verify.user.id;
    var squery =
      "select * from dealer_pagination where dealer_id = " +
      dealer_id +
      " AND type ='" +
      dropdownType +
      "'";
    ...
    if (srslt.length == 0) {
      var squery = sql.query(
        "insert into dealer_pagination (dealer_id, record_per_page, type) values (" +
          dealer_id +
          ", '" +
          selectedValue +
          "', '" +
      ...
      sql.query(
        "update dealer_pagination set record_per_page = '" +
          selectedValue +
          "' where type='" +
          dropdownType +
          "' AND dealer_id='" +
...
router.get("/dealer/getPagination/:dropdownType", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    sql.query(
      "select record_per_page from dealer_pagination where dealer_id = " +
        dealer_id +
        " AND type = '" +
        dropdownType +
        "'",
    ...
    sql.query(
      ...
      function(err, rslts) {
        ...
        if (rslts.length == 0) {
          ...
          if (
            rslts[0].selected_items != "" &&
            rslts[0].selected_items != null
          ) {
...
router.get("/getinfo", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    var getinfo =
      "select * from dealers where dealer_id='" + verify.user.id + "'";
    ...
    sql.query(getinfo, async function(err, rows) {
      if (verify.user.user_type != "sdealer") {
        ...
        var gtdealername = await sql.query(
          "select * from dealers where dealer_id = '" +
            rows[0].connected_dealer +
            "'"
        );
        data = {
          status: true,
...
router.post("/admin/dropdown", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    if (!empty(req.body.selected_items)) {
      sql.query(
        "update users set selected_items = '" +
...
router.get("/admin/gtdropdown", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    sql.query(
      "select * from users where email = 'admin@gmail.com' and role = 'admin'",
      function(err, rslts) {
        if (rslts.length == 0) {
          data = {
            status: true,
          ...
          if (fieldName == "sim_ids") {
            ...
            parsedData.forEach(async row => {
              if (row.sim_id && row.start_date && row.expiry_date) {
                let result = await sql.query(
                  "INSERT IGNORE sim_ids (sim_id, start_date, expiry_date) value ('" +
                    row.sim_id +
                    "', '" +
                    row.start_date +
                    "', '" +
            ...
            if (!error) {
              ...
              res.send({
                status: true,
                msg:
                  "File contained invalid data that has been ignored, rest has been imported successfully.",
                data: []
              });
            }
            ...
            parsedData.forEach(async row => {
              if (row.chat_id) {
                let result = await sql.query(
                  "INSERT IGNORE into chat_ids (chat_id) value ('" +
                    row.chat_id +
                    "')"
                );
                console.log(result);
            ...
            parsedData.forEach(async row => {
              if (row.pgp_email) {
                ...
                if (email) {
                  let result = await sql.query(
                    "INSERT IGNORE into pgp_emails (pgp_email) value ('" +
                      row.pgp_email +
                      "')"
                  );
                  console.log(result);
...
router.post("/upload", function(req, res) {
  ...
  if (verify.status == true) {
    ...
    if (req.body.logo !== "" && req.body.apk !== "" && req.body.name !== "") {
      sql.query(
        "insert into apk_details (app_name,logo,apk,created) values ('" +
          req.body.name +
          "' , '" +
          req.body.logo +
          "' , '" +
...
router.get("/apklist", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    sql.query(
      "select * from apk_details where delete_status=0 order by id ASC",
      async function(error, results) {
        if (error) throw error;
        ...
        if (results.length > 0) {
          ...
          for (var i = 0; i < results.length; i++) {
            let permissions =
              results[i].dealers !== undefined && results[i].dealers !== null
                ? JSON.parse(results[i].dealers)
                : "[]";
            let permissionCount =
              ...
              permissions !== "[]"
                ...
                : 0;
            let permissionC =
              dealerCount == permissionCount && permissionCount > 0
                ? "All"
...
router.post("/save_permissions", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    ...
    let updateAPKQ =
      "UPDATE apk_details set dealers = '" + dealers + "' WHERE id=" + apkId;
...
router.post("/edit/apk", function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    sql.query(
      "update apk_details set app_name = '" +
        req.body.name +
        "', logo = '" +
        req.body.logo +
        "', apk = '" +
...
router.post("/apk/delete", function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    if (!empty(req.body.apk_id)) {
      sql.query(
        "update `apk_details` set delete_status='1' WHERE id='" +
          req.body.apk_id +
          "'",
        async function(error, results) {
          console.log(results);
          ...
          if (results.affectedRows == 0) {
            ...
            let deletedRecord =
              "SELECT * FROM apk_details where id=" +
              req.body.apk_id +
              " and delete_status='1'";
...
router.delete("/delete_profile/:profile_id", async function(req, res) {
  ...
  if (verify.status !== undefined && verify.status == true) {
    if (!empty(req.params.profile_id)) {
      sql.query(
        "delete from device_history WHERE id=" + req.params.profile_id,
...
cron.schedule("0 0 0 * * *", async () => {
  ...
  for (var i = 0; i < results.length; i++) {
    if (
      formatted_dt == results[i].expiry_date ||
      formatted_dt > results[i].expiry_date
    ) {
      var update_sqll =
        "update devices set status = 'expired' where device_id ='" +
        results[i].device_id +
        "'";
